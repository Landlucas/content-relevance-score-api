...
with LazyImport(
    message="Run 'pip install farm-haystack[inference]'"
    ...
    **Usage example at index time:**
    ...
    def predict(
        ...
        texts = [
            ...
            else doc.meta[self.classification_field]
        ...
        for batch in batches:
            if self.task == "zero-shot-classification":
                batched_prediction = self.model(
                    batch, candidate_labels=self.labels, truncation=True
                )
            elif self.task == "text-classification":
                batched_prediction = self.model(
                    batch, top_k=self.top_k, truncation=True
                )
    ...
    def predict_batch(
        ...
        if isinstance(documents[0], Document):
            ...
            number_of_documents = [
                len(doc_list) for doc_list in documents if isinstance(doc_list, list)
            ]
